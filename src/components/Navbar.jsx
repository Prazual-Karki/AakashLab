...
const Navbar = () => {
  ...
  return (
    <div className='sticky top-0 left-0 z-10 bg-[#FFFFFF]'>
      <>
        {/* mobile view < 640px*/}
        <nav className='sm:hidden flex flex-row justify-between py-[17px] px-[15px] sm:px-[40px] items-center'>
        ...
        <div
          className={
            isSidebar
              ? 'flex flex-col overflow-x-hidden  h-full w-full py-[30px] px-[20px] fixed z-[999] left-0 top-0 right-0   bg-white transition-all duration-500'
              : 'hidden'
          }
        >
          <div
          ...
          {menuList.map((menu, i) => {
            return (
              <Link
                ...
                onClick={() => setIsSidebar(false)}
              >
                <p className='font-[600] text-[15px] tracking-wide leading-[17px]'>
                  {menu.name}
                </p>
              </Link>
            )
